...
export const InfiniteMovingCards = ({
  ...
  return (
    ...
    >
      ...
      >
        {items.map((item, idx) => (
          <li className="relative max-w-2xl py-2 shrink-0 " key={idx}>
            <blockquote>
              <div
                aria-hidden="true"
                className="user-select-none pointer-events-none absolute -top-0.5 -left-0.5 -z-1 h-[calc(100%_+_4px)] w-[calc(100%_+_4px)]"
              ></div>
              <span className="imc-icon-theme relative -z-20 text-sm leading-[1.6] font-normal">
                {item.icon}
              </span>
              <div className="relative -z-20 mt-6 flex flex-row items-center justify-center">
                <span className="flex items-center justify-center gap-1">
                  <span className="imc-title-theme text-sm leading-[1.2] font-normal">
                    {item.title}
                  </span>
                </span>
              </div>
